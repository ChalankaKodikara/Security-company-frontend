...
const View_Month_End_Medical = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.log("Medical data fetched:", json);
        ...
        if (e.name !== "AbortError") {
          console.error("Failed to load medical allowances:", e);
          setFetchError("Failed to load medical allowances.");
          setRows([]);
          setTotal(0);
          setTotalPages(1);
        }
  ...
  //  PUT Complete (medical_allowance)
  const handleComplete = async () => {
    ...
    try {
      ...
      if (res.ok && json?.success !== false) {
        toast.success(
          json?.message || "Medical allowances marked as complete.",
        );
      } else {
        toast.error(json?.message || "Failed to complete medical allowances.");
      }
    } catch {
  ...
  return (
    <div className="mx-auto mt-5 font-montserrat">
      <p className="text-[24px] mb-4">
        Medical Allowances · {year}-{String(month).padStart(2, "0")}
      </p>
...
export default View_Month_End_Medical;

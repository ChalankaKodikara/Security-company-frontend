...
const View_Month_End_Allowance = () => {
  ...
  /* -------------------- Fetch Employees with Allowances -------------------- */
  useEffect(() => {
    if (!orgId) return;
  ...
  const columns = useMemo(() => {
    ...
    for (const emp of rows) {
      for (const a of emp?.allowances || []) {
        const key = a?.actual_column_name;
        if (!key || !/^allowance_/i.test(key)) continue;
        if (!map.has(key)) map.set(key, a?.suggested_name || key);
      }
    }
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (res.ok && json?.success !== false) {
        toast.success(json?.message || "Allowances marked as complete.");
      } else {
        toast.error(json?.message || "Failed to complete allowances.");
      }
    } catch {
          ...
          <thead className="bg-white text-gray-600 border-b border-t">
            <tr>
              ...
              {isLoading ? (
                ...
                <th className="p-3 text-gray-400">No allowance columns</th>
              ) : (
                columns.map((c) => (
              ...
              rows.map((r) => (
                <tr key={r.employee_no} className="border-b hover:bg-gray-50">
                  ...
                  {columns.length === 0 ? (
                    ...
                    columns.map((c) => {
                      const entry = (r.allowances || []).find(
                        (a) => a?.actual_column_name === c.actual_column_name,
                      );
                      return (
...
export default View_Month_End_Allowance;
